<?php
/**
 * Created by PhpStorm.
 * User: Wellison
 * Date: 18/11/2017
 * Time: 14:39
 */

//sou seguido por follower
//print_r($followed_follower);
//echo '<br>';
foreach ($followed as $chave => $id) {

    $teste = array($chave => $id);
//    print_r($teste);

}
//echo '<br>';
$intersect = array_intersect($followed, $followers);
foreach ($intersect as $new_key => $ident_user) {
    $newvar[$new_key] = $ident_user['id'];

}
//print_r($newvar);

//echo '<br>';
//print_r($quant_seguidores);
//var_dump($followed_follower);
//foreach ($id as $i) {
//    print_r($i);
//    echo '<br>';
////    print_r($chave);
//}

?>
<div class="container">

    <div class="row">
        <div class="col-md-8 col-md-offset-2 col-sm-offset-1 col-sm-10">

            <div class="panel panel-default">
                <div class="panel-heading">
                    <div class="row">
                        <div class="col-md-6 text-center">
                            <h3><a href="#tab_seguindo" data-toggle="tab"
                                   id="li_seguindo">Seguidores <br> <?php echo @$quant_seguidores ?></a></h3></div>
                        <div class=" col-md-6 text-center">
                            <h3><a href="#tab_seguidores" data-toggle="tab"
                                   id="li_seguidores">Seguindo<br> <?php echo @$quant_seguindo ?></a></h3>
                        </div>
                    </div>
                </div>

                <div class="panel-body">
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_seguindo">
                            <div class="text-left">
                                <h3>Seguidores</h3>
                            </div>
                            <div class="row">
                                <form class="result_followed">
                                    <?php
                                    //                                    echo form_open('Pesquisar/listar', array('id' => 'formulario', 'method' => 'post', 'role' => 'form'));
                                    foreach ($followers as $key2 => $follow) {
//                                        foreach ($followed as $key3 => $follow_as) {
//                                            if (in_array($follow['id'], $followed)) {
//                                                $key4 = $key3;
//                                            }

//                                        echo '<br>';
                                        $this->fetchuser->setauthtoken($this->session->userdata('verify')['auth_token']);
                                        $this->fetchuser->setiduser($follow['id']);
                                        $this->fetchuser->setuserattributes($follow['id'], $this->fetchuser->getauthtoken());
                                        $image = $this->fetchuser->getuserimage();
                                        $name = $this->fetchuser->getusername();

                                        if ($name) {
                                            $return_name = $name;
                                        } else {

                                            $return_name = 'Not name Not name';
                                        }

                                        if (in_array($follow['id'], $followed_follower)) {
                                            $texto = 'Seguindo';
                                            $class = 'btn btn-primary ' . $follow['id'];
                                        } else {
                                            $texto = 'Seguir';
                                            $class = 'btn btn-default ' . $follow['id'];
                                        }
                                        $primary = 'btn btn-primary ' . $follow['id'];
                                        $default = 'btn btn-default ' . $follow['id'];
                                        if (!$image) {
                                            $image_final = 'data:image/png;base64,';
                                        } else {
                                            $image_final = $image;
                                        }

                                        $intersect_2 = array_intersect($newvar, $follow);
                                        $chave_unfollow = key($intersect_2);
                                        ?>
                                        <div class="col-md-4">
                                            <div class="jumbotron">
                                                <div class="text-center">
                                                    <img src="<?php echo $image_final; ?>"
                                                         class="img-circle"
                                                         width="50%"
                                                         height="50%"><br>
                                                    <b> <?php echo $return_name ?><br></b>
                                                    <!--                -->
                                                    <?php //echo $user['attributes']['age'] ?><!-- Anos</b><br>-->
                                                    <button type="button"
                                                            class="<?php echo $texto . $key2 ?> <?php echo $class ?>" value="<?php echo $texto ?>">
                                                        <?php echo $texto ?>
                                                    </button>

                                                </div>
                                            </div>
                                        </div>
                                        <script>
                                            $(document).ready(function () {
                                                $(".<?php echo $texto . $key2 ?>").bind('click', function () {
                                                    var id = "<?php echo $chave_unfollow;?>";
                                                    var id_user = "<?php echo $follow['id'];?>";
                                                    var texto = $(".<?php echo $texto . $key2 ?>").val();
//                                                    alert(texto);
//                                                    die();
                                                    $.post('follows_funtion/', {
                                                        iduser: id_user,
                                                        idfollow: id,
                                                        funcao: texto
                                                    }, function () {
                                                        if (texto == 'Seguir') {
                                                            $(".<?php echo $texto . $key2 ?>").removeClass("<?php echo $class?>");
                                                            $(".<?php echo $texto . $key2 ?>").addClass("<?php echo $primary?>");
                                                            $(".<?php echo $texto . $key2 ?>").html('Seguindo');
                                                            $(".<?php echo $texto . $key2 ?>").val('Seguindo');

//                                                            texto = 'Seguindo';
//                                                        } else {
//                                                            $
//
                                                        }
                                                        if (texto == 'Seguindo') {
                                                            $(".<?php echo $texto . $key2 ?>").removeClass("<?php echo $class?>");
                                                            $(".<?php echo $texto . $key2 ?>").addClass("<?php echo $default?>");
                                                            $(".<?php echo $texto . $key2 ?>").html('Seguir');
                                                            $(".<?php echo $texto . $key2 ?>").val('Seguir');

//                                                            texto = 'Seguir';
//
//
                                                        }

                                                    });
                                                });
                                            });
                                        </script>
                                        <?php
                                    }
                                    //                                    }
                                    ?>
                            </div>
                        </div>


                        <div class="tab-pane" id="tab_seguidores">
                            <div class="text-left">
                                <h3>Seguindo</h3>
                            </div>
                            <div class="row">
                                <div class="result_follower">
                                    <?php
                                    foreach ($followed as $key1 => $follow_as) {
                                        $this->fetchuser->setauthtoken($this->session->userdata('verify')['auth_token']);
                                        $this->fetchuser->setiduser($follow_as['id']);
                                        $this->fetchuser->setuserattributes($follow_as['id'], $this->fetchuser->getauthtoken());
                                        $image = $this->fetchuser->getuserimage();
                                        $name = $this->fetchuser->getusername();

                                        if ($name) {
                                            $return_name = $name;
                                        } else {

                                            $return_name = 'Not name Not name';
                                        }

                                        $texto = 'Seguindo';
                                        $class = 'btn btn-primary ' . $follow_as['id'];


                                        if (!$image) {
                                            $image_final = 'data:image/png;base64,';
                                        } else {
                                            $image_final = $image;
                                        }
                                        ?>
                                        <div class="col-md-4">
                                            <div class="jumbotron">
                                                <div class="text-center">
                                                    <img src="<?php echo $image_final; ?>"
                                                         class="img-circle"
                                                         width="50%"
                                                         height="50%"><br>
                                                    <b> <?php echo $return_name ?><br></b>
                                                    <!--                -->
                                                    <?php //echo $user['attributes']['age'] ?><!-- Anos</b><br>-->
                                                    <button type="button"
                                                            class="<?php echo $class ?> <?php echo $texto . $key1 ?>">
                                                        <?php echo $texto ?>
                                                    </button>

                                                </div>
                                            </div>
                                        </div>
                                        <script>
                                            $(document).ready(function () {
                                                var texto = "<?php echo $texto;?>";
                                                var id = "<?php echo $key1;?>";
                                                $(".<?php echo $texto . $key1 ?>").bind('click', function () {
                                                        alert(texto);
                                                        alert(id);
                                                    }
                                                );
                                            });
                                        </script>
                                        <?php
                                    }

                                    ?>
                                </div>

                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
