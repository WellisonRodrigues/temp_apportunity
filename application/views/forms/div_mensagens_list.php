...
//            print_r($retorno);
            ...
            <div class="media msg message" id="<?php echo $retorno['id'] ?>">
                ...
                <a class="pull-right" href="#">
                    <i class="fa fa-trash delete<?php echo $retorno['id'] ?>"></i>
                    <!--                    <input type="hidden" class="--><?php //echo $idmessage ?><!--" value="-->
                    <?php //echo $idmessage ?><!--">-->
                </a>
            ...
            <script>
                $(document).ready(function () {
                    $('.delete' + '<?php echo $retorno['id']?>').bind('click', function () {
                        if (confirm('deseja excluir?') == true) {
                            $.get('Mensagens/delete_message/<?php echo $retorno['id']?>', function (data) {
                                $("#<?php echo $retorno['id']?> ").remove();
                            })
                        }
                    });
                });
    ...
    <div class="btn-panel">
        ...
        <!--                    </i> Add Files</a>-->
        <button type="button" class=" col-lg-4 text-right btn  btn-primary send-message-btn pull-right"
                role="button">
...
<script>
    $(document).ready(function () {
//        $('.msg-wrap').animate({scrollBottom: 0}, "slow");
        $(".msg-wrap").animate({scrollTop: $('.msg-wrap').prop("scrollHeight")}, 1000);
        $('.send-message-btn').bind('click', function () {
            var text = $('textarea').val();
            $.post('Mensagens/create_message', {text: text, idchat:<?php echo $idchat?>}, function (data) {
                $("#new_div").html(data);
            },);
            $('textarea').val('');
            $(".msg-wrap").animate({scrollTop: $('.msg-wrap').prop("scrollHeight")}, 1000);
